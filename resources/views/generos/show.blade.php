...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-8">
        <div class="flex items-start justify-between mb-4">
            <div class="flex-1">
                <h1 class="text-3xl font-bold text-gray-800 mb-2">{{ $genero->nombre }}</h1>
                @if($genero->descripcion)
                <p class="text-lg text-gray-600">{{ $genero->descripcion }}</p>
                @endif
            </div>
            <div class="px-6 py-3 bg-primary-light/20 rounded-lg ml-6">
                <p class="text-sm text-gray-600">Total de libros</p>
                <p class="text-3xl font-bold text-primary">{{ $genero->libros->count() }}</p>
            </div>
        </div>

        <div class="flex gap-3 pt-6 border-t border-gray-200">
            @if(auth()->user()->isAdmin())
            <a href="{{ route('generos.edit', $genero) }}" class="px-6 py-3 bg-primary text-white rounded-lg hover:bg-primary-light transition-colors">
                Editar Género
            </a>
            @endif
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Libros de {{ $genero->nombre }}</h2>
        ...
        <div class="text-center py-12">
            ...
            <p class="text-gray-500 text-lg">No hay libros en este género aún</p>
            @if(auth()->user()->isAdmin())
