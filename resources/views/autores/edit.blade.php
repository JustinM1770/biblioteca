...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('autores.update', $autor) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="date" name="fecha_nacimiento" id="fecha_nacimiento" value="{{ old('fecha_nacimiento', $autor->fecha_nacimiento?->format('Y-m-d')) }}"
                       max="{{ date('Y-m-d') }}"
            ...
            <div class="flex gap-3 pt-4">
                ...
                <form method="POST" action="{{ route('autores.destroy', $autor) }}" class="ml-auto" onsubmit="return confirm('¿Estás seguro de eliminar este autor? Se eliminarán también todos sus libros.')">
                    @csrf
                    @method('DELETE')
